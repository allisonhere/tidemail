	...
	sections := []section{
		...
		},
		{
			...
			entries: []entry{
				...
				{keys.Space.Help().Key, "set the selected account as the default sender (★)"},
				{keys.MoveAccountUp.Help().Key + "/" + keys.MoveAccountDown.Help().Key, "move the selected account up or down the list"},
				{"Default sender", "the ★ account is the From address for a new message, and the account focused at startup"},
				{"Account order", "the account list order is the sidebar order and the ctrl+u sender order"},
		...
		},
		{
			...
			entries: []entry{
				...
				{"t", "test selected rule on its accounts' inboxes without changing mail"},
				{"r", "run selected rule on its accounts' inboxes"},
				{"a", "run all rules on all mail"},
				{"move", "new destination folders are created when the rule is saved"},
				{"J/K", "move selected rule down/up in priority"},
				{"Review: ctrl+s/enter", "save generated rule"},
				{"Review: r/a/e", "save+run, save+run all, or edit text"},
			},
		},
		{
			name: "Settings Modal",
