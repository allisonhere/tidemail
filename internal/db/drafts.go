...
func (db *DB) SaveDraft(d Draft) (int64, error) {
	...
	if id == 0 {
		...
		if _, err := tx.Exec(`
			UPDATE drafts SET
				account_name = ?, account_user = ?, account_index = ?, mailbox_id = ?,
				remote_uid = ?, remote_message_id = ?, to_addr = ?, cc_addr = ?,
				subject = ?, body_text = ?, in_reply_to = ?, references_text = ?,
				created_at = ?, updated_at = ?, last_remote_sync = ?, dirty = ?
			WHERE id = ?`,
			d.AccountName, d.AccountUser, d.AccountIndex, d.MailboxID,
			d.RemoteUID, d.RemoteMessageID, d.To, d.CC,
			d.Subject, d.BodyText, d.InReplyTo, d.References,
			createdAt, updatedAt, lastRemoteSync, dirty, id); err != nil {
			return 0, err
		}
	}
